...
function start() {
  ...
  if (characterDataToSort.length < 2) {
    alert('Cannot sort with less than two characters. Please reselect. 排行時不能少於兩個角色，請檢查角色範圍');
    return;
  }
  ...
  preloadImages().then(() => {
    ...
    document.querySelectorAll('.sort.text, .sort.spacer').forEach(el => el.style.display = 'flex');
    display();
  });
}
...
function display() {
  ...
  const charSeriesDisp = seriesArr => {
    if (!Array.isArray(seriesArr) || !seriesArr.length) {
      return '';
    }
    const fullSeries = seriesArr.join(' / ');
    const seriesText = reduceTextWidth(fullSeries, 'Arial 12.8px', 220);
    const seriesTooltip = seriesText !== fullSeries ? fullSeries : '';
    return `<p title="${seriesTooltip}">${seriesText}</p>`;
  };
  ...
  const leftSpacer = document.querySelector('.left.sort.spacer');
  const rightSpacer = document.querySelector('.right.sort.spacer');
  const leftSeries = charSeriesDisp(leftChar.opts ? leftChar.opts.series : []);
  const rightSeries = charSeriesDisp(rightChar.opts ? rightChar.opts.series : []);
  leftSpacer.innerHTML = leftSeries;
  rightSpacer.innerHTML = rightSeries;
  leftSpacer.style.display = leftSeries ? 'flex' : 'none';
  rightSpacer.style.display = rightSeries ? 'flex' : 'none';
...
function result(imageNum = 20) {
  ...
  document.querySelectorAll('.sort.text, .sort.spacer').forEach(el => el.style.display = 'none');
  ...
  const headElem = resultTable.querySelector('.result.head');
  let lastOffsetTop = headElem ? headElem.offsetTop : null;
  const appendResultRow = (markup) => {
    resultTable.insertAdjacentHTML('beforeend', markup);
    const rowElem = resultTable.lastElementChild;
    if (lastOffsetTop !== null && rowElem.offsetTop < lastOffsetTop) {
      rowElem.insertAdjacentHTML('beforebegin', header);
    }
    lastOffsetTop = rowElem.offsetTop;
  };
  ...
  characterDataToSort.forEach((val, idx) => {
    ...
    if (imageDisplay-- > 0) {
      appendResultRow(imgRes(character, rankNum));
    } else {
      appendResultRow(res(character, rankNum));
    }
...
function generateImage() {
  ...
  const width = resultsElem.scrollWidth;
  const height = resultsElem.scrollHeight;
  const viewportWidth = document.documentElement.clientWidth;
  const viewportHeight = document.documentElement.clientHeight;

  html2canvas(resultsElem, {
    width,
    height,
    windowWidth: Math.max(width, viewportWidth),
    windowHeight: Math.max(height, viewportHeight)
  }).then(canvas => {
